namespace PptxTemplater
{
    ...
    internal static class PptxParagraph
    {
        ...
        internal static bool ReplaceTag(A.Paragraph p, string tag, string newText)
        {
            ...
            {
                // Search for the tag
                string strTextP = GetTexts(p);
                Match match = Regex.Match(strTextP, PrepareExactPatternForRegex(tag));
                if (!match.Success)
                {
                    break;
                }
                ...
                for (int i = 0; i < texts.Count; i++)
                {
                    ...
                    if (match.Index >= text.StartIndex && match.Index <= text.EndIndex)
                    {
                        ...
                        for (; i < texts.Count; i++)
                        {
                            ...
                            List<char> currentTextChars = new(currentText.Text.Text.ToCharArray());
        ...
        private static string PrepareExactPatternForRegex(string input)
        {
            string strSpecials = "\\[]/^$.|?*+(){}";
            foreach(char c in strSpecials)
            {
        ...
        /// </summary>
        private class TextIndex(A.Text t, int startIndex)
        {
            public A.Text Text { get; private set; } = t;
