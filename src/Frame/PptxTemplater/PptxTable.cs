namespace PptxTemplater
{
    ...
    public class PptxTable
    {
        ...
        {
            A.Table? tbl = slideTemplate.FindTable(tblId);
            if(tbl == null)
            {
                return;
            }
        ...
        {
            if (backgroundPicture == null || backgroundPicture.Content == null)
            {
                return;
            }
            ...
            A.BlipFill blipFill = new();
            A.Blip blip = new() { Embed = slide.GetIdOfImagePart(imagePart) };
            A.SourceRectangle srcRect = new();
            A.Stretch stretch = new();
            A.FillRectangle fillRect = new()
                {
        ...
        {
            PptxSlide slide = slideTemplate;
            A.Table? tbl = slide.FindTable(tblId);
            ...
            if(tbl != null)
            {
                ...
                int done = 1;
                for (int i = 0; i < rows.Count; i++)    // Было .Count()
                {
                    Cell[] row = rows[i];
        ...
        public IEnumerable<string> ColumnTitles()
        {
            ...
            A.Table? tbl = slideTemplate.FindTable(this.tblId);
            if(tbl != null)
            {
        ...
        {
            A.Table? tbl = slideTemplate.FindTable(this.tblId);
            return (tbl != null) ? CellsCount(tbl) : 0;
        }
