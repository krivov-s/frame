namespace PptxTemplater
{
    ...
    public class PptxSlide
    {
        ...
        {
            return slidePart.Slide.Descendants<A.Paragraph>().Select(p => PptxParagraph.GetTexts(p));
        }
        ...
        public string GetTitle()
        {
            ...
            Shape? titleShape = slidePart.Slide.Descendants<Shape>().FirstOrDefault(sp => IsShapeATitle(sp));
            if (titleShape != null)
            {
        ...
        public IEnumerable<string> GetNotes()
        {
            ...
            {
                notes.AddRange(slidePart.NotesSlidePart.NotesSlide.Descendants<A.Paragraph>().Select(p => PptxParagraph.GetTexts(p)));
            }
            return notes;
        }
        ...
        public IEnumerable<PptxTable> GetTables()
        {
            ...
            int tblId = 0;
            foreach (GraphicFrame graphicFrame in slidePart.Slide.Descendants<GraphicFrame>())
            {
                NonVisualDrawingProperties? cNvPr = graphicFrame.NonVisualGraphicFrameProperties?.NonVisualDrawingProperties;
                if (cNvPr != null && cNvPr.Title != null)
                {
        ...
        {
            return GetTables().Where(table => table.Title.Contains(tag));
        }
        ...
        {
            foreach (A.Paragraph p in slidePart.Slide.Descendants<A.Paragraph>())
            {
        ...
        {
            ReplaceTag(tagPair.Tag, tagPair.NewText, replacementType);
        }
        ...
        public void ReplacePicture(string tag, byte[] newPicture, string contentType)
        {
            ...
            ImagePart imagePart = AddPicture(newPicture, contentType);
        ...
        public void ReplacePicture(string tag, string newPictureFile, string contentType)
        {
            ...
            ReplacePicture(tag, bytes, contentType);
        }
        ...
        {
            SlidePart slideTemplate = slidePart;
            ...
            // Copy layout part
            if(slideTemplate.SlideLayoutPart != null)
            {
            ...
            return new PptxSlide(presentationPart, slidePartClone);
        }
        ...
        public static void InsertAfter(PptxSlide newSlide, PptxSlide prevSlide)
        {
            ...
            if(presentationPart != null)
            {
                ...
                if(slideIdList != null)
                {
                    ...
                    // Find the highest id
                    uint maxSlideId = slideIdList.ChildElements.Cast<SlideId>().Max(x => x.Id?.Value ?? 0); // было просто x.Id.Value
        ...
        public void Remove()
        {
            ...
            if(slideIdList != null)
            {
                foreach (SlideId slideId in slideIdList.ChildElements.Cast<SlideId>())
                {
                    if (slideId.RelationshipId == presentationPart.GetIdOfPart(slidePart))
                    {
        ...
        private static bool IsShapeATitle(Shape sp)
        {
            ...
            PlaceholderShape? ph = sp.NonVisualShapeProperties?.ApplicationNonVisualDrawingProperties?.GetFirstChild<PlaceholderShape>();
            if (ph != null && ph.Type != null && ph.Type.HasValue)
            {
                if((PlaceholderValues)ph.Type == PlaceholderValues.Title || ((PlaceholderValues)ph.Type == PlaceholderValues.CenteredTitle))
                {
                    isTitle = true;
                }
            }
        ...
        internal ImagePart AddPicture(byte[] picture, string contentType)
        {
            ...
            PartTypeInfo type = default;
            //ImagePartType type = default;
            switch (contentType)
            {
                case "image/bmp":
        ...
        internal A.Table? FindTable(int tblId)
        {
            ...
            IEnumerable<GraphicFrame> graphicFrames = slidePart.Slide.Descendants<GraphicFrame>();
